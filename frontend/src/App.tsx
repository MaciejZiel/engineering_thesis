...
export default function App(){
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <nav aria-label="Main navigation">{tabs.map(([id,label,Icon])=><button key={id} aria-label={label} className={`nav-item ${tab===id?'selected':''}`} aria-current={tab===id?'page':undefined} onClick={()=>{setTab(id);setFocus(false);}}><Icon size={17}/><span>{label}</span>{id==='sessions'&&recording&&<i className="dot red"/>}</button>)}</nav>
      ...
      <button className="nav-item" aria-label="Preferences" title="Preferences" onClick={()=>setModal('settings')}><Settings2 size={17}/><span>Preferences</span></button>
      <button className="nav-item" aria-label="Keyboard shortcuts" title="Keyboard shortcuts" onClick={()=>setModal('help')}><CircleHelp size={17}/><span>Keyboard shortcuts</span><kbd>?</kbd></button>
          ...
          <div className={`workspace ${focus?'focused':''}`}>
            <section className="scene-panel" aria-label="Robot digital twin"><div className="panel-toolbar"><div className="panel-title"><Box size={16}/><h2>Digital twin</h2><span className="subtle-tag">SIMULATED</span></div><button className="icon-button" aria-label={focus?'Show camera panel':'Focus digital twin'} title="Focus view (V)" onClick={()=>setFocus(!focus)}><Focus size={16}/></button></div>
              <div className="scene-container"><Suspense fallback={<div className="scene-placeholder">Loading 3D workspace…</div>}><Scene state={state} view={view} grid={prefs.grid} targets={prefs.targets} body={prefs.body} reset={reset}/></Suspense>
                ...
                {!ready&&state?.arms&&<div className="stale-label" role="status">Last known pose · live data unavailable</div>}<div className="scene-topline"><span><span className="legend-line left"/>Left arm</span><span><span className="legend-line right"/>Right arm</span></div>
