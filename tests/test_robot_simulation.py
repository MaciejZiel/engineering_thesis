...
class SimulationBackendTests(unittest.TestCase):
    def test_both_arms_start_at_home_with_open_grippers(self) -> None:
        ...
        for arm in state.arms.values():
            self.assertEqual(arm.joints, {"shoulder": 45.0, "elbow": 45.0, "wrist": 45.0})
    ...
    def test_joint_moves_at_most_max_speed_per_step(self) -> None:
        ...
        backend.send(right({"shoulder": 180.0}))

        state = backend.step(0.5)

        self.assertAlmostEqual(state.arm("right").joints["shoulder"], 135.0)
        self.assertEqual(state.arm("right").targets["shoulder"], 180.0)
    ...
    def test_joint_reaches_target_without_overshoot(self) -> None:
        backend = SimulationBackend(RobotConfig(max_speed_deg_s=90.0), clock=FakeClock())
        backend.send(right({"wrist": 100.0}))

        state = backend.step(1.0)

        self.assertEqual(state.arm("right").joints["wrist"], 100.0)
    ...
    def test_send_uses_clock_to_advance_motion(self) -> None:
        ...
        clock.now = 2.0
        backend.send(right({"shoulder": 0.0}, ts=2))

        self.assertAlmostEqual(backend.state.arm("right").joints["shoulder"], 70.0)
    ...
    def test_targets_are_clamped_to_joint_limits(self) -> None:
        ...
        self.assertEqual(backend.state.arm("right").targets["elbow"], 150.0)
    ...
    def test_status_lines_describe_both_arms(self) -> None:
        backend = SimulationBackend(RobotConfig(), clock=FakeClock())
        backend.send(right({"shoulder": 120.0}))

        lines = backend.status_lines()

        self.assertEqual(lines[0], "sim R: S  90.0->120.0 E  90.0-> 90.0 W  90.0-> 90.0 grip open")
        self.assertEqual(lines[1], "sim L: S  90.0-> 90.0 E  90.0-> 90.0 W  90.0-> 90.0 grip open")
